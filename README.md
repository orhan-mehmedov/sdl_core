...
**Note:** It is important that both sources and binaries are in one and the same directory. The name of directory is not important. It is important it is associated with `SDL_SQLITE_DIR`.
...
* Go to the directory with the cloned repository and enter the subfolder tools\ms.
...
* After script execution finish:
  ...
  * Go to the build directory which is located one directory upper and open smartDeviceLinkCore.sln file using Visual Studio 2013
